...
### 3. Deploying to Render with Supabase & Custom Subdomain

A `render.yaml` blueprint is included. Set these environment variables in the Render dashboard:
- `DATABASE_URL`: Your Supabase connection string (`postgresql://postgres:...@db....supabase.co:5432/postgres`)
- `FLASK_SECRET_KEY` (auto-generated by the blueprint)
- `ADMIN_USERNAME` / `ADMIN_PASSWORD` — your staff credentials

#### Connecting Your Custom Subdomain on Render:
1. In the Render Dashboard, go to your Web Service -> **Settings** -> **Custom Domains**.
2. Add your subdomain: `attendance.yourdomain.com`.
3. In your DNS provider (Cloudflare, GoDaddy, Namecheap), create a `CNAME` record:
   - **Type**: `CNAME`
   - **Name**: `attendance`
   - **Target**: your Render URL (e.g. `cyber-attend.onrender.com`)
4. Render automatically provisions a free SSL (HTTPS) certificate.

#### Keeping Render Awake 24/7 (For 3-Day Events):
To prevent Render Free from sleeping after 15 minutes of inactivity:
1. Sign up on **[UptimeRobot](https://uptimerobot.com/)** (Free).
2. Create an **HTTP Monitor** targeting `https://attendance.yourdomain.com`.
3. Set the interval to **5 minutes**.
4. Render will stay active 24/7 without any cold-start delays!

### 4. Provisioning Student IDs & PINs in Bulk

Generate or import student credentials into Supabase before the event:
```bash
# Generate 50 student credentials with default PIN:
python import_students.py --generate 50 --event-code TECHSTAR-26 --prefix TECHSTAR-26 --default-pin 2026

# Or create a blank CSV template:
python import_students.py --template

# Import from your existing CSV:
python import_students.py --file your_students.csv --event-code TECHSTAR-26
```
This automatically saves a handout CSV (`student_credentials_techstar-26.csv`) with Participant IDs and PINs to distribute to students via WhatsApp or Email.
